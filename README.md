...
California, with discrete field measurements, plus an interactive offline viewer.
...
| Path | What it is |
|---|---|
| `ca_groundwater_explorer.html` | **Interactive viewer** — open directly in any browser (no server needed). Keep the `data/` folder next to it. |
...
| `download_ca_groundwater.py` | Downloads everything from USGS (re-run to refresh). |
| `build_viewer_data.py` | Rebuilds the viewer data files from the CSVs. |

## Dataset summary

- California groundwater wells with continuous (daily-values) water-level records,
  led by parameter **72019** (with elevation series 62610/62611 at many wells) (depth to water below land surface, ft)
...
- Data tables (annual / monthly / percentiles), light/dark themes,
  deep links: `ca_groundwater_explorer.html#site=<well number>`

## Refreshing the data

```
python download_ca_groundwater.py   # refreshes the well catalog + re-downloads everything through today
